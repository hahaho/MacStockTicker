...
from PyQt5.QtCore import Qt, QTimer, QPoint, pyqtSignal, QPropertyAnimation, QRect
...
class StockInfo:
    ...
    @property
    def color(self):
        if self.change > 0: return "#ff4c4c" # 红色
        if self.change < 0: return "#32cc32" # 绿色
        return "#ffffff"
...
class MarqueeWidget(QWidget):
    ...
    def update_data(self, stocks):
        ...
        for _ in range(2):
            for stock in stocks:
                label = QLabel(f"{stock.name} {stock.price:.2f} {stock.percent_change:+.2f}%")
                label.setStyleSheet(f"color: {stock.color}; font-weight: bold; font-family: Consolas;")
...
class StockItemWidget(QFrame):
    ...
    def init_ui(self):
        ...
        add_indicator("低", f"{self.stock.low:.2f}", "#32cc32")
        ...
        lbl_vol.setStyleSheet("font-size: 11px; background: transparent;")
        ...
        self.del_btn.setStyleSheet("QPushButton { background-color: #ff4c4c; color: white; border-radius: 10px; font-weight: bold; } QPushButton:hover { background-color: #ff0000; }")
...
class MainWindow(QWidget):
    ...
    def init_ui(self):
        ...
        min_btn.setStyleSheet("color: white; background: transparent; font-weight: bold;")
        min_btn.setCursor(QCursor(Qt.PointingHandCursor))
        min_btn.clicked.connect(lambda: self.toggle_mode(True))
        
        close_btn = QPushButton("×")
        close_btn.setFixedSize(24, 24)
        close_btn.setStyleSheet("color: #ff4c4c; background: transparent; font-size: 18px; font-weight: bold;")
        ...
        self.input_field.setStyleSheet("background-color: rgba(0,0,0,100); color: white; border: none; border-radius: 5px; padding: 5px;")
        self.input_field.returnPressed.connect(self.add_stock)
        
        add_btn = QPushButton("+")
        add_btn.setFixedSize(28, 28)
        add_btn.setStyleSheet("color: white; background: rgba(0,0,0,100); border-radius: 14px; font-weight: bold;")
        ...
        self.scroll_area.setStyleSheet("QScrollArea { border: none; background: transparent; } QScrollBar { width: 0px; }")
        ...
        exp_btn.setStyleSheet("color: white; background: transparent; font-weight: bold; font-size: 16px;")
        ...
        m_close_btn.setStyleSheet("color: #ff4c4c; background: transparent; font-weight: bold; font-size: 18px;")
    ...
    def toggle_mode(self, is_minimized, animate=True):
        ...
        if is_minimized:
            ...
            target_width, target_height = 800, 50
        else:
            self.expanded_widget.show()
            self.minimized_widget.hide()
            target_width, target_height = 300, 500

        # Anchor to bottom right concept (if screen is known, but here we anchor to current right edge)
    ...
    def paintEvent(self, event):
        ...
        if self.config.is_minimized:
            ...
            painter.setBrush(QColor(30, 30, 32, 240))
            painter.setPen(QColor(255, 255, 255, 40))
    ...
    def update_ui(self):
        ...
        if self.config.is_minimized:
            ...
            for stock in self.all_stocks:
                ...
                # Don't allow deleting indices
                if stock.symbol in self.config.index_symbols:
                    widget.del_btn.deleteLater()
                else:
...
if __name__ == '__main__':
    ...
    # Optional: Initial position to bottom right
    screen = app.primaryScreen().geometry()
    window.move(screen.width() - window.width() - 50, screen.height() - window.height() - 100)
